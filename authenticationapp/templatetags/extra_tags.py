...
def split(value):
    process_name = [each_name.title() for each_name in value.split(' ')[:len(value.split(' ')) - 1]]
...
def benchmark(value):
    ...
    benchmark_name_splitted = [each_bnc for each_bnc in benchmark_name.split('_')]
    return " ".join(benchmark_name_splitted)
...
def top_ram(value):
    ...
    if str(top_ram_name).endswith("(ram)"):
        return True
    else:
        return False
...
def top_cpu(value):
    ...
    if str(top_cpu_name).endswith("(cpu)"):
        return True
    else:
        return False
