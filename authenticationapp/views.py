...
def index(request):
    ...
    events = TblEvent.objects.all().order_by("-event_value")
...
def viewSites(request, id):
    ...
    clientName = TblClients.objects.filter(client_id=id)[0].client_name
    # pagination
    ...
    try:
        ...
        sites = paginator.get_page(page)
    # defining context dictionary
    context = {'sites': sites, 'clientName':clientName, 'paginator':paginator}
...
def viewAllSites(request):
    ...
    try:
        ...
        sites = paginator.get_page(page)
    # defining context dictionary
    context = {'sites': sites, 'paginator':paginator}
...
def viewSystems(request, id):
    ...
    # getting all entries in sites table
    siteName = TblSites.objects.filter(site_id=id)[0].site_name
    # pagination
    ...
    try:
        ...
        systems = paginator.get_page(page)
    # defining context dictionary
    context = {'systems': systems, 'siteName':siteName, 'paginator':paginator}
...
def viewAllSystems(request):
    ...
    try:
        ...
        systems = paginator.get_page(page)
    # defining context dictionary
    context = {'systems': systems, 'paginator':paginator}
...
def systemDetail(request, id):
    ...
    events = TblEvent.objects.all().filter(system_id=id).order_by('-event_creation_date')
    ...
    ram_bnc_id = TblBenchmark.objects.get(bnc_process="system_ram_usage").bnc_id

    # Getting benchmark id of average cpu value 
    cpu_bnc_id = TblBenchmark.objects.get(bnc_process="system_cpu_usage").bnc_id

    try:
        # getting latest entry of average ram value
        ram_value = TblEvent.objects.all().filter(bnc_id=ram_bnc_id, system_id = id).order_by('-event_creation_date')[0].event_value

        # getting latest entry of average cpu value
        cpu_value = TblEvent.objects.all().filter(bnc_id=cpu_bnc_id, system_id = id).order_by('-event_creation_date')[0].event_value
    except IndexError:
        ram_value = ""
        cpu_value = ""

    #getting latest 5 entries for average ram value to be displsyed on graph
    ram_graph = TblEvent.objects.all().filter(system_id=id, bnc_id=ram_bnc_id).order_by('-event_creation_date')[:5][::-1]
    
    #getting latest 5 entries for average cpu value to be displsyed on graph
    cpu_graph = TblEvent.objects.all().filter(system_id=id, bnc_id=cpu_bnc_id).order_by('-event_creation_date')[:5][::-1]


    # defining context dictionary
    context = {'benchmark': benchmark, 'events':events, 'ram_graph':ram_graph,'cpu_graph':cpu_graph, 'system':system, 'ram_value':ram_value, 'cpu_value':cpu_value}
